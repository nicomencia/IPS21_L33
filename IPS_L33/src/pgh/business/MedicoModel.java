...
import pgh.util.ApplicationException;
import pgh.jdbc.Database;
import pgh.util.Util;
/**
...
public class MedicoModel {
	 ...
	 */
	public List<MedicoDisplayDTO> getListaCarreras(Date fechaInscripcion) {
		validateNotNull(fechaInscripcion,MSG_FECHA_INSCRIPCION_NO_NULA);
		String sql=
				 "SELECT id,descr,"
				+" case when ?<inicio then ''" //antes de inscripcion
				...
				+" from carreras  where fecha>=? order by id";
		String d=Util.dateToIsoString(fechaInscripcion);
		return db.executeQueryPojo(MedicoDisplayDTO.class, sql, d, d, d, d, d);
	}
	/** 
	 ...
	 */
	public MedicoEntity getCarrera(int id) {
		String sql="SELECT id,inicio,fin,fecha,descr from carreras where id=?";
		List<MedicoEntity> carreras=db.executeQueryPojo(MedicoEntity.class, sql, id);
	...
	public void updateFechasInscripcion(int id, Date inicio, Date fin) {
		MedicoEntity carrera=this.getCarrera(id);
